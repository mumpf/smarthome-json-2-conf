...
namespace json2conf
{
    class ExtractGA
    {
        ...
        private void ExtractEtsGA(string iFileName)
        {
            ...
            XDocument xElement = XDocument.Load(iFileName);
            foreach (var lElement in xElement.Descendants("{http://knx.org/xml/project/11}GroupAddress"))
            {
        ...
        public bool ExtractEtsProject(string iEtsProjectFileName)
        {
            ...
            if (Path.GetExtension(iEtsProjectFileName) == ".knxproj")
            {
                ...
                string lTempFileName = Path.Combine(lTempPath, "0.xml");
